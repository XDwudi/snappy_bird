...
## v1.4.0（进行中 · 批次1，2026-09-12）
> 能力扩展包批次1：13 张新卡（common×6 + uncommon×7），卡池 28→41；rare/epic×14 + 旧卡质变补丁为批次2
> 设计依据：docs/开发方案_v1.4.0.md §2.1/§2.2/§2.6（r2 定稿）；迭代文档：docs/iterations/迭代_v1.4.0_能力扩展包.md
...
- 回归：三模拟器通过；专项断言 42 项全过（求生本能 HP=1 触发与次数上限 / 连击种子保留上限与硬刹车 / 镜面护盾 3s 刹车 / 经验银行上限+升级取出+生息封顶 / 补给线保底计时与独立性 / 铁喙反杀免伤+Boss免疫+教学一次 / 导弹挂架 MAX_ALIVE 挂钩+扇形 / 定风珠免疫期风力归零与恢复 / effectText 13 卡全对）；gameplay_sim 随机基线中位 63.8s→62.3s（≥红线 60.5s 守住）；builds_sim 没胡陷阱流中位 46.8s→52.1s（新 common 支点抬下限，符合 N6 意图），天胡流派因池稀释略降（132.5→115.0，§6.3 全指标批次2 验收）
